...
class Emitter():
    ...
    *   @param in the type of the local array variable.
    ...
    def emitNEWARRAY(self, in_, frame):
        ...
        frame.push()
        typ = in_
        eleType, dimen = typ.eleType, typ.dimen
        if type(eleType) is cgen.ArrayType:
        ...
        elif type(eleType) is cgen.StringType:
            result.append(self.emitPUSHICONST(dimen, frame))
            frame.pop()
    ...
    *   @param in the list of symbol entries corresponding to local array variable.
    '''
    def emitPUSHACONST(self, in_, typ, frame):
        # in_: ArrayLiteral
        # typ: ArrayType
        # frame: Frame
        ...
        result.append(self.emitNEWARRAY(typ, frame))
        
        def emitELEMENT(in_, typ, index, frame):
            result.append(self.emitDUP(frame))
            ele = in_.value[index]
            if type(typ) is cgen.ArrayType:
                result.append(self.emitPUSHICONST(index, frame))
                result.append(self.emitNEWARRAY(typ, frame))
                result.append(self.emitASTORE(typ, frame))
                result.append(self.emitDUP(frame))
                result.append(self.emitPUSHICONST(index, frame))
                result.append(self.emitALOAD(typ, frame))
                eleType = typ.eleType
                list(map(lambda x: emitELEMENT(ele, eleType, x, frame), range(typ.dimen)))
                result.append(self.emitPOP(frame))
            else:
                result.append(self.emitPUSHICONST(index, frame))
                result.append(self.emitPUSHCONST(str(ele.value).lower() if type(typ) is cgen.BoolType else ele.value, typ, frame))
                result.append(self.emitASTORE(typ, frame))
  
        list(map(lambda x: emitELEMENT(in_, typ.eleType, x, frame), range(typ.dimen)))
        return ''.join(result)
